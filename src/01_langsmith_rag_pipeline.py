...
def setup_vectorstore():
    ...
    """
    # Implemented: Khởi tạo embeddings từ factory (1 dòng)
    embeddings = get_embeddings()

    # Implemented: Đọc nội dung knowledge base (1 dòng)
    text = load_knowledge_base()

    # Implemented: Chia text thành chunks với chunk_size=500, chunk_overlap=50 (1 dòng)
    chunks = split_text(text, chunk_size=500, chunk_overlap=50)
    print(f"📚 Đã chia thành {len(chunks)} chunks")

    # Implemented: Tạo FAISS vectorstore và trả về (1 dòng)
    vectorstore = build_vectorstore(chunks, embeddings)
    return vectorstore


# ── 2. RAG Prompt Template ─────────────────────────────────────────────────
# Implemented: Tạo ChatPromptTemplate với 2 messages:
...
def build_rag_chain(vectorstore):
    ...
    # Implemented: Tạo retriever từ vectorstore, lấy k=3 tài liệu gần nhất
    # Gợi ý: retriever = vectorstore.as_retriever(search_kwargs={"k": 3})
    retriever = vectorstore.as_retriever(search_kwargs={"k": 3})

    # Implemented: Định nghĩa hàm format_docs để ghép page_content của các docs thành 1 chuỗi
    # Gợi ý: "\n\n".join(doc.page_content for doc in docs)
    def format_docs(docs):
        return "\n\n".join(doc.page_content for doc in docs)

    # Implemented: Xây dựng LCEL chain dùng pipe operator (|)
    # Gợi ý:
    #   chain = (
    #       {"context": retriever | format_docs, "question": RunnablePassthrough()}
...
# Implemented: Thêm decorator @traceable(name="rag-query", tags=["rag", "step1"])
...
def ask(chain, question: str) -> str:
    ...
    """
    # Implemented: Gọi chain.invoke(question) và trả về kết quả
...
def main():
    ...
    # Implemented: Gọi setup_vectorstore() để tạo vectorstore
    vectorstore = setup_vectorstore()

    # Implemented: Gọi build_rag_chain(vectorstore) để nhận chain và retriever
    chain, retriever = build_rag_chain(vectorstore)

    # Implemented: Lặp qua tất cả SAMPLE_QUESTIONS, gọi ask(), in câu hỏi và câu trả lời
