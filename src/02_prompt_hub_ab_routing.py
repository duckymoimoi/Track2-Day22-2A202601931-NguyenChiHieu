...
# Implemented: Đổi thành tên của bạn — phải là duy nhất trong Hub của bạn
PROMPT_V1_NAME = "nguyen-chi-hieu-2a202601931-rag-v1"
PROMPT_V2_NAME = "nguyen-chi-hieu-2a202601931-rag-v2"


# ── 2. Định nghĩa 2 Prompt Templates ──────────────────────────────────────
# Implemented: Viết SYSTEM_V1 — phong cách ngắn gọn, trả lời 2-4 câu
...
SYSTEM_V1 = (
    ...
    "không bịa thêm thông tin. Trả lời trực tiếp, ngắn gọn trong 2-4 câu. "
...
# Implemented: Viết SYSTEM_V2 — phong cách có cấu trúc, expert tone, 3-5 câu
# Gợi ý: "Bạn là chuyên gia AI. Đọc kỹ context, xác định facts liên quan,
#          viết câu trả lời rõ ràng và có tổ chức (3-5 câu). ..."
SYSTEM_V2 = (
    "Bạn là chuyên gia về hệ thống AI. Hãy đọc kỹ context, xác định "
    "các fact liên quan rồi trả lời rõ ràng, có cấu trúc trong 3-5 câu. "
    "Chỉ suy luận từ context, không dùng kiến thức bên ngoài; nếu thiếu dữ kiện "
    "thì nêu rõ giới hạn thay vì đoán.\n\nContext:\n{context}"
)
...
def push_prompts_to_hub(client: Client):
    ...
    """
    # Implemented: Push PROMPT_V1 — bọc trong try/except để xử lý lỗi
    try:
        url = client.push_prompt(
            PROMPT_V1_NAME, object=PROMPT_V1, description="V1 - concise RAG answer"
        )
        print(f"✅ Đã push V1 → {url}")
    except Exception as e:
        if "Nothing to commit" in str(e):
            print(f"✅ V1 đã cập nhật trên Hub (không có thay đổi mới)")
        else:
            print(f"⚠️  V1 lỗi: {e}")

    # Implemented: Push PROMPT_V2 — bọc trong try/except
    try:
        url = client.push_prompt(
            PROMPT_V2_NAME, object=PROMPT_V2, description="V2 - structured expert RAG answer"
        )
        print(f"✅ Đã push V2 → {url}")
    except Exception as e:
        if "Nothing to commit" in str(e):
            print(f"✅ V2 đã cập nhật trên Hub (không có thay đổi mới)")
        else:
            print(f"⚠️  V2 lỗi: {e}")
...
def pull_prompts_from_hub(client: Client) -> dict:
    ...
    # Implemented: Pull PROMPT_V1_NAME, fallback về PROMPT_V1 nếu lỗi
    try:
    ...
    # Implemented: Pull PROMPT_V2_NAME, fallback về PROMPT_V2 nếu lỗi
    try:
...
def get_prompt_version(request_id: str) -> str:
    ...
    """
    # Implemented: Tính MD5 hash của request_id và chuyển thành số nguyên
    hash_int = int(hashlib.md5(request_id.encode()).hexdigest(), 16)

    # Implemented: Trả về PROMPT_V1_NAME nếu chẵn, PROMPT_V2_NAME nếu lẻ
    return PROMPT_V1_NAME if hash_int % 2 == 0 else PROMPT_V2_NAME


# ── 6. Traced A/B Query ────────────────────────────────────────────────────
# Implemented: Thêm @traceable(name="ab-rag-query", tags=["ab-test", "step2"])
...
def ask_ab(retriever, llm, prompt, question: str, version: str) -> dict:
    ...
    """
    # Implemented: Retrieve docs từ retriever
    docs = retriever.invoke(question)

    # Implemented: Ghép page_content thành 1 string (dùng "\n\n".join)
    context = "\n\n".join(doc.page_content for doc in docs)

    # Implemented: Chạy chain và lấy answer
    answer = (prompt | llm | StrOutputParser()).invoke(
        {"context": context, "question": question}
    )

    # Implemented: Trả về dict kết quả
    return {
        "question": question,
        "context": context,
        "answer": answer,
...
def main():
    ...
    # Implemented: Tạo LangSmith Client với API key từ config
    # Gợi ý: client = Client(api_key=config.LANGSMITH_API_KEY)
    client = Client(api_key=config.LANGSMITH_API_KEY)

    # Implemented: Push cả 2 prompts lên Hub
    push_prompts_to_hub(client)

    # Implemented: Pull cả 2 prompts từ Hub (dùng dict trả về)
    prompts = pull_prompts_from_hub(client)

    # Tạo vectorstore, retriever và LLM
    vectorstore = setup_vectorstore()
    # Implemented: Tạo retriever từ vectorstore (k=3)
    ...
    for i, question in enumerate(SAMPLE_QUESTIONS):
        ...
        # Implemented: Lấy version key từ request_id qua get_prompt_version()
        version_key = get_prompt_version(request_id)
        version_tag = "v1" if version_key == PROMPT_V1_NAME else "v2"
        prompt      = prompts[version_key]

        # Implemented: Gọi ask_ab() với đúng arguments
