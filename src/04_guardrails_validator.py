...
class PIIDetector(Validator):
    ...
    def validate(self, value: str, metadata: dict):
        ...
        # Implemented: Lặp qua self.PII_PATTERNS.items()
        for pii_type, pattern in self.PII_PATTERNS.items():
            # Implemented: Tìm tất cả matches
            matches = re.findall(pattern, value)

            for match in matches:
                # Implemented: Thay thế match bằng "[PII_TYPE_REDACTED]" trong redacted_text
        ...
        if found_pii:
            ...
            # Implemented: Trả về PassResult với value_override=redacted_text
            return FailResult(
                error_message="Phát hiện PII trong output.",
                fix_value=redacted_text,
            )

        # Implemented: Không có PII → trả về PassResult với value gốc
...
class JSONFormatter(Validator):
    ...
    def _repair(text: str) -> str:
        ...
        # Implemented: Thay single quotes → double quotes
        text = text.replace("'", '"')

        # Implemented: Xóa trailing commas (dùng re.sub với r',\s*([}\]])' → r'\1')
    ...
    def validate(self, value: str, metadata: dict):
        ...
        """
        # Implemented: Thử parse JSON trực tiếp
        try:
            parsed = json.loads(value)
            # Implemented: Trả về PassResult với json.dumps(parsed, indent=2)
            return PassResult(value_override=json.dumps(parsed, indent=2))
        except json.JSONDecodeError:
            pass

        # Implemented: Thử sửa JSON rồi parse lại
        try:
            ...
            fallback = json.dumps(
                {"error": "invalid_json", "original": value}, indent=2
            )
            return FailResult(
...
def demo_pii_guard():
    ...
    # Implemented: Tạo Guard với PIIDetector, truyền on_fail=OnFailAction.FIX vào CONSTRUCTOR
    ...
    for label, text in test_cases:
        # Implemented: Gọi guard.validate(text) để lấy ValidationOutcome
...
def demo_json_guard():
    ...
    # Implemented: Tạo Guard với JSONFormatter, truyền on_fail=OnFailAction.FIX vào CONSTRUCTOR
